...
- Features são packages Flutter reais: devem possuir `pubspec.yaml`, uma
  biblioteca pública em `lib/<feature_name>.dart` e uma dependência de caminho
  correspondente no `pubspec.yaml` da aplicação. O código da feature não pode
  importar `package:rickandmorty_app` nem depender do package da aplicação, para
  evitar ciclo com `core`.
